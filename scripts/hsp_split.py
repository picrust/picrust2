...
from picrust2.default_split import default_tables_bac, default_tables_arc

HSP_METHODS = ['mp', 'emp_prob', 'pic', 'scp', 'subtree_average']

TRAIT_OPTIONS = ['16S', 'BIGG', 'CAZY', 'EC', 'GENE_NAMES', 'GO', 'KO', 'PFAM']
...
def main():
    ...
    if args.in_trait and args.observed_trait_table:
        ...
        if args.reference in ['bac', 'bacteria']: default_tables = default_tables_bac
        elif args.reference in ['arc', 'archaea']: default_tables = default_tables_arc
        else:
          raise RuntimeError(
