    ...
    if args.in_trait and args.observed_trait_table:
        ...
        if args.database != 'SC':
            if args.database == 'oldIMG': 
                ...
                raise RuntimeError(
                          "Unknown option set for -db/--database. Valid options are oldIMG or SC. See the help documentation on the wiki for further information.")
        else:
            if args.in_trait not in TRAIT_OPTIONS_NEW:
                    RuntimeError(
                      "You've chosen a train option that's not available for the new SC database. Available options are: "+','.join(TRAIT_OPTIONS_NEW))
